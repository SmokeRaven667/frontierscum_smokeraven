...
const INDIVIDUAL_INITIATIVE_ROLL_CARD_TEMPLATE =
  "systems/frontierscum/templates/chat/individual-initiative-roll-card.html";
const PARTY_INITIATIVE_ROLL_CARD_TEMPLATE =
  "systems/frontierscum/templates/chat/party-initiative-roll-card.html";
...
export const rollPartyInitiative = async () => {
  ...
  if (initiativeRoll.total <= 3) {
    outcomeText = game.i18n.localize("FS.InitiativeEnemiesBegin");
  } else {
    outcomeText = game.i18n.localize("FS.InitiativePlayerCharactersBegin");
  }
...
export const rollIndividualInitiative = async (actor) => {
  if (game.combats && game.combat) {
    ...
    if (combatant) {
      ...
      ui.notifications.warn(`${game.i18n.localize("FS.ActorNotInEncounter")}!`);
    }
    return;
  }
...
export class FSCombat extends Combat {

...
export class FSItem extends Item {
  ...
  prepareDerivedData() {
    ...
    if (this.type === CONFIG.FS.itemTypes.armor) {
      this.data.data.damageReductionDie =
        CONFIG.FS.armorTiers[this.data.data.tier.value].damageReductionDie;
    }
  }
  ...
  prepareActorItemDerivedData(actor) {
    if (actor.type === "character" || actor.type === "follower") {
      this.data.data.equippable = CONFIG.FS.equippableItemTypes.includes(
        this.type
      );
      this.data.data.droppable =
        CONFIG.FS.droppableItemTypes.includes(this.type) &&
        this.data.data.carryWeight !== 0;
      this.data.data.canPlusMinus = CONFIG.FS.plusMinusItemTypes.includes(
        this.type
      );
    } else {
  ...
  get isContainer() {
    return this.type === CONFIG.FS.itemTypes.container;
  }

  get isEquipment() {
    return CONFIG.FS.itemEquipmentTypes.includes(this.type);
  }

  get isContainerizable() {
    return CONFIG.FS.allowedContainerItemTypes.includes(this.type);
  }

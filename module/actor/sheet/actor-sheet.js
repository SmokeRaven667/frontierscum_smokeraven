...
export default class FSActorSheet extends ActorSheet {
  ...
  async _onItemCreate(event) {
    ...
    const template = "systems/frontierscum/templates/dialog/add-item-dialog.html";
    const dialogData = {
    ...
    return new Promise((resolve) => {
      new Dialog({
        title: game.i18n.localize("FS.CreateNewItem"),
        content: html,
        buttons: {
          create: {
            icon: '<i class="fas fa-check"></i>',
            label: game.i18n.localize("FS.CreateNewItem"),
            callback: (html) =>
  ...
  async _onItemDelete(event) {
    ...
    if (item.isContainer && item.hasItems) {
      Dialog.confirm({
        title: game.i18n.localize("FS.ItemDelete"),
        content: "<p>" + game.i18n.localize("FS.ItemDeleteMessage") + "</p>",
        yes: async () => {
